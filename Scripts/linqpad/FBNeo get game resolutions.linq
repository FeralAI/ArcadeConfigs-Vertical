...
const string RomPath = @"C:\ROMs\fbneo";
const string ConfigPath = @"C:\Emulators\RetroArch\config\FinalBurn Neo";
const string NewConfigPath = @"C:\Emulators\RetroArch\config\FinalBurn Neo (new)";
...
async Task Main()
{
	...
	foreach (var romFile in romFiles)
	{
		...
		string configFile = Path.Join(ConfigPath, $"{name}");
		...
		else
		{
			...
			config.UseRunahead = true;
			config.RunAheadFrames = 1;
		}
